...
def calculate_metrics(preds, labels, top_k=(1, 3, 5)):
    ...
    for k in top_k:
        if k > preds.size(1): continue
        correct = (pred_indices[:, :k] == labels.view(-1, 1)).any(dim=1).float()
        metrics[f'acc@{k}'] = correct.mean().item() * 100
...
def main():
    ...
    visualize_tsne(test_features, test_labels, class_names=class_names, 
                  title="Linear Probe Feature Visualization")
