...
model = ViTWithProjection(num_classes=num_classes).to(device)

# 冻结除投影层和分类头外的 ViT 参数
for name, param in model.named_parameters():
    if not ("projection" in name or "classifier" in name):
...
optimizer = torch.optim.AdamW(
    filter(lambda p: p.requires_grad, model.parameters()),  # 只优化投影层和分类头
    lr=1e-4,
    weight_decay=1e-4
)
