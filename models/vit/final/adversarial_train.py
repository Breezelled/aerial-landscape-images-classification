...
class TrainableBlackDotAttacker(nn.Module):
    def __init__(self, image_shape=(3, 224, 224), epsilon=10):
        ...
        # 一个小型 CNN，从输入 x 生成 delta mask（共享通道）
        self.cnn = nn.Sequential(
            nn.Conv2d(C, 32, kernel_size=3, padding=1),  # (B, 32, H, W)
            nn.ReLU(),
        
            nn.Conv2d(32, 64, kernel_size=3, padding=1), # (B, 64, H, W)、
            nn.ReLU(),
        
            nn.Conv2d(64, 64, kernel_size=3, padding=1), # (B, 64, H, W)
            nn.ReLU(),
        
            nn.Conv2d(64, 32, kernel_size=3, padding=1), # (B, 32, H, W)
            nn.ReLU(),
    ...
    def start_weight(self):
        for m in self.cnn.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.uniform_(m.weight, a=-2, b=1)
                if m.bias is not None:
                    nn.init.uniform_(m.weight, a=-2, b=1)

    def forward(self, x):
        delta = self.cnn(x).squeeze(1)  # shape: (B, H, W)
        mask = torch.sigmoid(delta * self.epsilon).unsqueeze(1)  # shape: (B, 1, H, W)
        x_adv = x * mask
        return x_adv, mask

attacker = TrainableBlackDotAttacker(epsilon=epsilon).to(device)
attacker.start_weight()
...
for epoch in range(0,epochs):
    ...
    if epoch>=0: # epooch%2==1 and epoch>0:
        ...
        for i, (x, y, _) in enumerate(train_loader):
            x, y = x.to(device), y.to(device)
            x_adv, mask = attacker(x)
            outputs = vit(x_adv)
    
            # regularization
            # print(mask.size())
            loss_reg = -mask.sum() / mask.size(0) / mask.size(1)/ mask.size(2) / mask.size(3)
            ...
            # lambda_reg = loss_miss.item()/loss_reg.item()
...
torch.save(attacker.state_dict(), 'black_dot_attacker.pth')
