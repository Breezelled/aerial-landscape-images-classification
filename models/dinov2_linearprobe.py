...
model_name = "facebook/dinov2-base"  # DINOv2 base model
...
class DINOv2LinearProbeLogitWrapper(nn.Module):
    def __init__(self, dino_model, linear_probe):
        super().__init__()
    ...
    def forward(self, pixel_values):
        ...
        """
        # Extract features using the frozen DINOv2 model
        with torch.no_grad():
            outputs = self.dino_model(pixel_values)
            # Get CLS token features - the first token's output
...
# --- Main Function for DINOv2 Linear Probing ---
def run_dinov2_linear_probing(
    dataset_dir="../data/Aerial_Landscapes",
    save_dir="../results/dinov2_linear_probe",
    batch_size=32,
    model_path=None,  # Custom DINOv2 model path if available
    epochs=10,
    lr=1e-3,
    ...
    # --- 1. Load DINOv2 Model (Feature Extractor) ---
    if model_path:
        print(f"Loading custom DINOv2 model from {model_path}")
        dino_model = AutoModel.from_pretrained(model_path).to(device)
    else:
    ...
    print("Freezing DINOv2 model parameters...")
    for param in dino_model.parameters():
        param.requires_grad = False
    dino_model.eval()  # Set DINOv2 to evaluation mode
    ...
    # DINOv2-base hidden size is 768
    feature_dim = dino_model.config.hidden_size
    ...
    for epoch in range(epochs):
        ...
        for batch in pbar:
            ...
            # Extract features (no gradients needed for DINOv2)
            with torch.no_grad():
                outputs = dino_model(**batch_inputs)
    ...
    torch.save({
        'linear_probe': best_model_state,
        'class_names': class_names,
        'val_acc': best_val_acc,
        'epochs': epochs,
        'dino_model_name': model_path if model_path else model_name
    }, os.path.join(save_dir, "dinov2_linear_probe_full.pth"))
    ...
    with torch.no_grad():
        for batch in tqdm(val_dataloader, desc="Evaluating"):
             ...
             outputs = dino_model(**batch_inputs)
             features = outputs.last_hidden_state[:, 0]
             features_normalized = features / features.norm(p=2, dim=-1, keepdim=True)

             logits = linear_probe(features_normalized)
             ...
             all_image_features.append(features.cpu().numpy())
    ...
    with open(result_txt_path, "w") as f:
        ...
        f.write(f"Base DINOv2 Model: {model_path if model_path else model_name}\n")
...
# --- GradCAM Functions (Adapted for DINOv2) ---

def vit_reshape_transform(tensor):
    # tensor: [B, seq_len, hidden_dim]
    B, N, C = tensor.shape
    # For ViT, the patch tokens start at index 1 (index 0 is CLS token)
    # Calculate grid size from sequence length (minus CLS token)
    h = w = int((N - 1) ** 0.5)
    # Reshape patch tokens to spatial arrangement
    result = tensor[:, 1:, :].reshape(B, h, w, C)
    # Change to channels-first format for GradCAM
    result = result.permute(0, 3, 1, 2).contiguous()
    return result


def run_gradcam_vit_linear_probe(dino_model, linear_probe, processor, image_pil):
    ...
    wrapper = DINOv2LinearProbeLogitWrapper(dino_model, linear_probe).to(device)
    wrapper.eval()
    ...
    # Target the layer *within the DINOv2 vision model*
    # Usually the last attention block
    try:
        # Target the last layer norm in the transformer
        target_layers = [dino_model.encoder.layer[-1].layernorm_after]
    except AttributeError:
        print("Could not find default target layer. Trying alternative.")
        try:
            # Try another common location in ViT models
            target_layers = [dino_model.layernorm]
        except AttributeError:
             print("Error: Cannot find suitable target layer for GradCAM in the ViT model.")
             return None
...
def visualize_gradcam(
    ...
    val_dataset,
    class_names,
    out_dir="../results/dinov2_linear_probe/gradcam_pred",
):
    ...
    for class_name, img_path in images_to_visualize.items():
        try:
            ...
            result_img = run_gradcam_vit_linear_probe(dino_model, linear_probe, processor, image_pil)
...
# --- t-SNE Function (Unchanged) ---
def visualize_tsne(
    features, labels, class_names, out_path="../results/dinov2_linear_probe/tsne_features.svg"
):
    ...
    try:
        ...
        plt.title("t-SNE of DINOv2 Image Features (Linear Probing)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate DINOv2 models using Linear Probing"
    )
    parser.add_argument(
        "--dataset_dir", type=str, default="../data/Aerial_Landscapes", help="Dataset directory"
    )
    parser.add_argument(
        "--base_save_dir", type=str, default="../results/dinov2_linear_probe", help="Base directory to save results"
    )
    parser.add_argument(
        "--batch_size", type=int, default=128, help="Batch size for training and evaluation"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model_path", type=str, help="Path to a custom DINOv2 model", default=None
    )
    args = parser.parse_args()

    print(f"\n\n===== Running Linear Probing with DINOv2 model =====")
    print(f"Results will be saved in: {args.base_save_dir}")
    ...
    visualize_gradcam(
        ...
        val_dataset,
        class_names,
        out_dir=os.path.join(args.base_save_dir, "gradcam_pred"),
    )
    ...
    visualize_tsne(
        ...
        image_labels,
        class_names,
        out_path=os.path.join(args.base_save_dir, "tsne_features_val.svg"),
    )

    print("All DINOv2 linear probing tasks completed!")