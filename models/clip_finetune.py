...
def parse_args():
    ...
    parser.add_argument(
        "--save_every", type=int, default=1, help="Save model every n epochs"
    )
    parser.add_argument(
        "--images_dir",
        type=str,
        default="../data/RSICD/RSICD_images",
        help="Directory with RSICD images",
    )
    parser.add_argument(
        "--json_path",
        type=str,
        default="../data/RSICD/annotations_rsicd/dataset_rsicd.json",
        help="Path to RSICD annotations",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="../results/clip_finetuned_trainer",
...
def collate_fn(examples):
    ...
    batch = {
        k: torch.stack([example[k] for example in examples])
        for k in keys
        if k != "return_loss"
    }
...
def fine_tune_clip_with_trainer(
    ...
    train_dataset = RSICDDataset(
        images_dir=images_dir, json_path=json_path, processor=processor, split="train"
    )

    val_dataset = RSICDDataset(
        images_dir=images_dir, json_path=json_path, processor=processor, split="val"
    )
...
def evaluate_clip_on_test(
    ...
    test_dataset = RSICDDataset(
        images_dir=images_dir, json_path=json_path, processor=processor, split="test"
    )
...
def visualize_retrieval_examples(
    ...
    example_indices = np.random.choice(
        len(test_dataset), min(num_examples, len(test_dataset)), replace=False
    )
    ...
    text_inputs = processor(
        text=all_captions, padding=True, truncation=True, return_tensors="pt"
    ).to(device)
    ...
    for i, idx in enumerate(example_indices):
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(retrieval_dir, f"example_{i+1}.png"), bbox_inches="tight"
        )
        plt.close()
